...
- [ ] Create `AgentProfile` type in `src/runtime/types.ts`
...
- [ ] Create `SpawnConfig` interface in `src/runtime/spawn-config.ts`
  - Fields match `AgentProfile` plus `profile?: string` and `overrides`

- [ ] Create `SpawnConfigBuilder` class in `src/runtime/spawn-config.ts`
...
- [ ] Write unit tests for `SpawnConfigBuilder`
...
- [ ] Fix `before_model_call` propagation in `agent-runtime.ts`
...
- [ ] Write test for hook model propagation
...
- [ ] Add `maxIterations` to `AgentConfig` in `agent-runtime.ts`
...
- [ ] Implement `toolTimeoutMs` in `executeToolCalls()`
...
- [ ] Create `spawn()` function in `src/runtime/spawn.ts`
...
- [ ] Create built-in profiles in `src/runtime/profiles.ts`
...
- [ ] Register built-in profiles at startup
...
- [ ] Update `docs/architecture.md` with profile system overview
- [ ] Add profile usage examples to agent definition files
- [ ] Update `agents/reviewer.md` to show profile-based spawning
